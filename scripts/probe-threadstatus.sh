...
scenarios=${THREADSTATUS_PROBE_SCENARIOS:-"startup work viewer sse sse-work mixed-stress"}
...
for scenario in $scenarios; do
  ...
  if env JOLT_CACHE_DIR="$jolt_cache" \
      ...
      "$jolt_wrapper" jolt -M:threadstatus-probe \
      "$scenario" "$port" >"$stdout_file" 2>"$stderr_file"; then
    process_status=0
  else
    process_status=$?
  fi
  if grep -Fq 'ThreadStatus: current_thread contains invalid address' "$stderr_file"; then
    diagnostic=present
  else
    diagnostic=absent
  fi
